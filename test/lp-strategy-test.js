...
const { increaseTime, overwriteTokenAmount, increaseBlock, returnSigner } = require("./utils/helpers");
const { expect } = chai;
const { setupSigners, snowballAddr, treasuryAddr, setupPenguin } = require("./utils/static");
...
const doLPStrategyTest = (name, _snowglobeAddr, _controllerAddr, globeABI, stratABI, _slot) => {
    ...
    describe("LP Strategy tests for: " + name, async () => {
        ...
        before(async () => {
            ...
            if (name.includes("Penguin")) await setupPenguin(iglooMaster);
        });
        ...
        async function setupPenguin(_addr, _strat) {
            // Impersonate IglooMaster and return signer
            const masterSigner = await returnSigner(_addr);
            await network.provider.send("hardhat_setBalance", [_addr,"0x10000000000000000000000",]);

            // Whitelist our strategy to interact with staking rewards
